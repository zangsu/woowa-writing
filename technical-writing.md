...
### DispatcherServlet
`DispatcherSerlvet` 의 가장 주요한 기능은 웹 요청들을 해당 요청을 처리할 수 있는 Handler 로 분배하는 것이다. 그리고, 이 역할이 `doDispatch()` 메서드로 구현이 되어 있다.

아래는 `doDispatch()` 메서드를 단순화 한 코드이다.
...
먼저, 사용자 요청을 처리할 수 있는 핸들러를 가져 온다. 이 때의 핸들러는 우리가 등록해 두었는 인터셉터 체인을 함께 포함한다.
...
마지막으로, 응답에 해당하는 view 를 반환하는 방법을 알아보자.
...
아래는 지금까지 알아본 `DispatcherServlet` 의 흐름을 간단하게 도식화 한 것이다. 보이는 것과 같이 여러 인터페이스들과 의존관계를 가지고 있다.

![img.png](img_spring_backbone.png)

## DispatcherServlet 의 실제동작

이번엔 `DispatcherServlet` 을 디버깅 하면서 실제 동작을 확인해 보자.
...
"localhost:8080/index" 로 요청을 보낸 후 DispatcherServlet 을 살펴보자.
...
`DispatcherServlet` 에 등록되어 있는 `HandlerMapping` 은 총 6개가 존재하는 것을 확인할 수 있다.
...
이후 `HandlerMethod` 형식의 핸들러를 지원할 수 있는 `RequestMappingHandlerAdapter` 를 찾아서 반환을 해 주는 것을 확인할 수 있다.
...
스프링은 OOP 를 잘 활용한 프레임워크로 유명하며, 이번 글에서도 그 점을 확인할 수 있다.
대부분의 협력 객체들은 인터페이스로 추상화 되어 있으며, 구현 클래스 역시 추상 클래스를 상속하는 구체 클래스가 대부분이다.
